param(
  ...
  [string]$VersionFile = "config.h",
  [string]$VersionDefine = "VK_FIRMWARE_VERSION",
...
function Get-FirmwareVersion([string]$VersionFilePath, [string]$DefineName) {
  if (-not (Test-Path -LiteralPath $VersionFilePath)) {
    Write-Error "Versionsdatei nicht gefunden: $VersionFilePath"
    exit 1
  }

  $pattern = '^\s*#define\s+' + [regex]::Escape($DefineName) + '\s+"([^"]+)"\s*$'
  $matches = Select-String -Path $VersionFilePath -Pattern $pattern
  if ($matches.Count -eq 0) {
    Write-Error "$DefineName wurde in $VersionFilePath nicht gefunden."
    exit 1
  }

  if ($matches.Count -gt 1) {
    Write-Error "$DefineName ist in $VersionFilePath mehrdeutig definiert."
    exit 1
  }
...
$versionFilePath = Join-Path $sketchPath $VersionFile
$softwareVersion = Get-FirmwareVersion -VersionFilePath $versionFilePath -DefineName $VersionDefine
...
$buildPath = Join-Path $boardRootPath "build"
...
New-Item -ItemType Directory -Force -Path $buildPath,$outputDirPath,$releaseDirPath,$logDirPath | Out-Null
...
Write-Host "[compile] Project: $ProjectName"
...
$process = Start-Process -FilePath $wrapperScriptFile -NoNewWindow -WorkingDirectory $repoRoot -PassThru
